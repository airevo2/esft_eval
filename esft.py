...
def load_base_model(base_model_path):
    print("Starting to load model on GPU...", flush=True)
    # load pretrained model directly on GPU
    ...
    print("Finished loading model on device:", next(model.parameters()).device, flush=True)
    # Note: The tokenizer is CPU based and does not require to be moved to GPU
    return model, tokenizer
