...
def eval_model(rank, args, model, dataset):
    ...
    try:
        ...
        if torch.cuda.is_available():
            model = model.to("cuda")
            print(f"Rank {rank}: CUDA is available.", flush=True)
            try:
                current_device = torch.cuda.current_device()
            except Exception as e:
                current_device = "Unknown"
            print(f"Rank {rank}: Current CUDA device: {current_device}", flush=True)
            first_param = next(model.parameters(), None)
            if first_param is not None:
                print(f"Rank {rank}: Model first parameter on device: {first_param.device}", flush=True)
            else:
